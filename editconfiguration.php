...
function writeLocalAdminHeader()
{
   ...
   {
      $phpinfo_mode = "false";
      $phpinfo_msg = "Edit Configuration";
   } else {
      $phpinfo_mode = "true";
      $phpinfo_msg = "System Installation Info";
   }
   */
...
function writePHPConfigInfo()
{
   ...
   <?
          writeInfoItem("Version:", $oscailt_basic_config['software_version'] );
   ...
   <?
      ...
      foreach ( $php_ext_array as $ext_key => $ext_name) 
      {
          ...
          writeInfoItem("PHP Extension ", $ext_name . $func_list );
...
function writeConfigForm()
{
   ...
   <input type=hidden name=memory_mgmt_activate_time value=<?=$system_config->memory_mgmt_activate_time?> >
   <?
      ...
      writeConfigTextAreaItem("Keywords","site_keywords",$system_config->site_keywords,"Comma separated keywords of the site. Also indexed by search engines");
      ...
      writeConfigItem("Banned SpamBot IP List","spam_ip_list",$system_config->spam_ip_list,"Enter what you think are Spam Bot IP addresses. Separate each one by spaces.");
      ...
      {
          writeInfoItem("<P class='error'>Warning: systemconfig.inc file value override is in effect for user_error reporting!</P>","");
      }
      ...
      writeConfigBooleanItem("Embedded Audio Player","audio_player_installed",$system_config->audio_player_installed,"Tick box if the XSPF Embedded Audio Player has been installed. This must be installed to allow embedded audio publishing.");
      if (extension_loaded('shmop') ) {
