...
//option to allow editor votes on comment and block hides when more than one vote. Voting down again can
//allow the hide option to reappear. Otherwise just go with straight hide and unhide.
$oscailt_basic_config['comment_vote_hide_block'] = false;

// Default to not install the any WYSISYG editor for publish page.
// The allowed values are: TinyMCE or OpenWYSIWYG
$oscailt_basic_config['enable_wysiwyg_editor'] = "";

//Default to hide embedded video or not. Some video hosts track the IP so by hiding video cover that can be
//avoided until you intentionally wish to view the video.
$oscailt_basic_config['embedded_video_default_mode'] = true;
//List of video types that are okay to display embedded html to allow video host to show video cover
//The video IDs must match values used in videos.inc
$oscailt_basic_config['safe_video_list'] = array(27, 10);

//List of video types that we do not add embedded video for automatically but retrieve default static cover image
//YouTube for example. They track IP so we get the cover image and cache it here. To add to this the cover image
//retrieval code has to be modifed as it is specific to each type. This list is used by the video gallery to retrieve
//cover images. So by adding an id to previous list, it will display actual video in story and cover image in gallery.
//The video IDs must match values used in videos.inc
$oscailt_basic_config['video_cover_image_retrieval'] = array(1,10, 27);
...
//some basic thumbnail image configurations (could be moved to object configurations)
$oscailt_basic_config['preserve_aspect_ratio'] = true;
