...
$textLabels = array("title" => "Manage Blocked Author Names",
	            "banned_author_subtitle" => "Banned Author Names (publish bans) &nbsp;  Time now: ",
	            ...
	            "ban_author_btn_label" => "Ban Author",
	            "no_valid_author_input_text" => "No valid Author Name was input!",
	            ); 

$textObj = new indyItemSet();
$system_config->user_error_reporting=8;
if($textObj->load($system_config->xml_store, "editbannedauthors") === false)
{
    $textObj->setUserMessage( array( USER_ERROR, "Failed to get text strings for the Edit Banned Authors. -Using defaults",""));
...
{
   global $authorBanList, $system_config, $textLabels;
   $timeMsg = strftime($system_config->default_strftime_format, time() + $system_config->timezone_offset);
   ?>
   <table align=center>
   <tr class=admin>
      <th class=admin colspan="6"><?=$textLabels['banned_author_subtitle']?> <?=$timeMsg?></td>
   </tr>
   <tr class=admin>
   <th class=admin>&nbsp;<?=$textLabels['author_name_label']?>&nbsp;</th>
   ...
   <?
	 ...
	 <input type="submit" name="go" value="<?=$textLabels['ban_author_btn_label']?>">
...
{
   writeAdminHeader("editbannedips.php","Ban IP Addresses and Sub Hosts", array("editmonitor.php" => "Monitor Publishers"));
   if($editor_session->editor->allowedReadAccessTo("editbannedips"))
   {
      ...
      if(isset($_REQUEST['subpage']) && isset($_REQUEST['confirm']))
      {
         ...
         {
            # No valid Author Name was input!
            writeError($textLabels['no_valid_author_input_text']);
