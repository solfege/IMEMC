...
// TBD - finish
$textLabels = array("title" => "Manage user roles and their permissions",
	            "show_who_has_roles" => "Show Who Has Roles",
	            "hide_who_has_roles" => "Hide Who Has Roles",
	            "roles_word" => "Roles",
	            "name_word" => "Name",
	            "type_word" => "Type",
	            ...
	            "no_access_word" => "No Access",
	            "read_only_word" => "Read Only",
	            "read_write_word" => "Read/Write",
	            "are_you_sure_text" => "Are you sure you wish to delete",
	            "cancelBtn" => "Cancel",
	            "deleteBtn" => "Delete",
	            "saveBtn" => "Save",
	            "specify_name_text" => "Please Specify Name",
	            "role_exists_text" => "A role with this name already exists!",
	            "no_role_text" => "No Role Type Specified!",
	            "saveBtn" => "Save");

$textObj = new indyItemSet();

if($textObj->load($system_config->xml_store, "editroles") === false)
{
    $textObj->setUserMessage( array( USER_ERROR, "Failed to get text strings for the Edit Roles. -Using defaults",""));
...
{
    global $roleList, $OSCAILT_SCRIPT, $textLabels;
    ...
    if ( isset($_REQUEST['show_who_has']) && $_REQUEST['show_who_has'] == 'true') {
       ...
       $swap_mode = $textLabels['hide_who_has_roles'];
       $switch_mode ="show_who_has=false";
       $who_has = true;
       $max_col = 7;
    } else  {
       // $swap_mode ="Show Who Has Roles";
       $swap_mode = $textLabels['show_who_has_roles'];
    ...
    <tr class=admin>
    <th class=admin colspan=<?=$max_col?>> <?=$textLabels['roles_word']?> </th>
    </tr>
    <tr class=admin>
        <th class=admin>&nbsp;#&nbsp;</th>
        <th class=admin>&nbsp;<a class='editor-option' href="<?=$OSCAILT_SCRIPT?>?sort_by_name=<?=$sort_by_name_mode?>"><?=$textLabels['name_word']?></a>&nbsp;</th> 
        <th class=admin>&nbsp;<a class='editor-option' href="<?=$OSCAILT_SCRIPT?>?sort_by_type=<?=$sort_by_type_mode?>"><?=$textLabels['type_word']?></a>&nbsp;</th> 
        <th class=admin>&nbsp;<?=$textLabels['description_word']?>&nbsp;</th>
    <?
    if($who_has == true) {
        ?>
    ...
    <tr class='admin'>
	   ...
	   <input type=submit name='create_role' value="<?=$textLabels['CreateNewBtn']?>">
...
{
   global $permissionsList, $path_prefix, $system_config, $textLabels;
...
{
    global $permissionsList, $textLabels;
    ...
    ?>
    <table align=center class=admin border=0>
    ...
    <tr class=admin>
        <td class=admin>&nbsp;<B><?=$textLabels['name_word']?></B>&nbsp;</td>
        <td class=admin colspan=3><input size=32 name=role_name value="<?=$role->role_name?>"></td>
    </tr>
    <tr class=admin>
            <td class=admin>&nbsp;<B><?=$textLabels['description_word']?></B>&nbsp;</td>
            <td class=admin colspan=3><input size=32 name=role_description value="<?=$role->role_description?>"></td>
    </tr>
    <tr class=admin>
        <th class=admin align=center>&nbsp;<?=$textLabels['permissions_word']?>&nbsp;</th>
        <th class=admin align=center>&nbsp;<?=$textLabels['no_access_word']?>&nbsp;</th>
        <th class=admin align=center>&nbsp;<?=$textLabels['read_only_word']?>&nbsp;</th>
        <th class=admin align=center>&nbsp;<?=$textLabels['read_write_word']?>&nbsp;</th>
    </tr>
...
function writeConfirmDeleteBox()
{
    ...
    ?>
    <tr>
	<td colspan=2 align=center><img src="graphics/caution.gif" align=middle><BR><BR><B><?=$textLabels['are_you_sure_text']?> <?=$role->role_name?>?</B><BR><BR></td>
    </tr>
    <tr>
...
if($editor_session->isSessionOpen())
{
    ...
    if($editor_session->editor->allowedReadAccessTo("editroles"))
    {
        ...
        else if(isset($_REQUEST['subpage']) && $_REQUEST['subpage']=="edit" && isset($_REQUEST['save']))
        {
            ...
            {
                writeError( $textLabels['specify_name_text']);
                writeEditBox(true);
            }

            else if($role->role_id==null && $roleList->getRoleByName($role->role_name)!=null)
            {
                writeError($textLabels['role_exists_text']);
                writeEditBox(true);
            }
            elseif($editor_session->editor->allowedWriteAccessTo("editroles"))
            {
                ...
                {
                   writeError($textLabels['no_role_text']);
                   writeRoleList();
                }
                $role->role_type = cleanseTitleField($_REQUEST['role_type']);
                if($role->role_type == 'site')
